...
app.post('/addEquipo', multer({ storage }).single('foto_equipo'), (req, res) => {

    const { descripcion_equipo, marca_equipo, tipo_equipo, tarifa_dia_equipo } = req.body;

    console.log('Datos del equipo:', req.body);

    const ruta_foto_equipo = req.file ? `../images/${req.file.filename}` : null;
    ...
    db_con.query(
        query,
        [ruta_foto_equipo, descripcion_equipo, marca_equipo, tipo_equipo, tarifa_dia_equipo],
        (err) => {
            if (err) {
    ...
    db_con.query(getPhotoQuery, [id_equipos], (error, results) => {
        ...
        if (results.length > 0) {
            ...
            db_con.query(deleteQuery, [id_equipos], (error, results) => {
                ...
                if (photoPath) {
                    fs.unlink(path.join(__dirname, photoPath), (err) => {
                        if (err) console.error('Error al eliminar la foto del equipo:', err);
                        else console.log('Foto del equipo eliminada:', photoPath);
                    });
                }
...
app.put('/updateEquipo/:id_equipos', multer({ storage }).single('foto_equipo'), (req, res) => {

    const { id_equipos } = req.params;
    const { descripcion_equipos, marca_equipos, tipo_equipos, tarifa_dia_equipos } = req.body;

    const newImagePath = req.file ? `../images/${req.file.filename}` : null;

    console.log('Nueva ruta de la imagen:', newImagePath);

    const getOldImageQuery = "SELECT foto_equipos FROM equipos WHERE id_equipos = ?";
    ...
    db_con.query(getOldImageQuery, [id_equipos], (err, results) => {
        ...
        if (newImagePath) {
            updateQuery = `UPDATE equipos SET
                                ...
                                WHERE
                                    id_equipos = ?`;
            queryParams = [newImagePath, descripcion_equipos, marca_equipos, tipo_equipos, tarifa_dia_equipos, id_equipos];
        } else {
            updateQuery = `UPDATE equipos SET
                                    descripcion_equipos = ?,
                                    marca_equipos = ?,
                                ...
                                WHERE
                                    id_equipos = ?`;
            queryParams = [descripcion_equipos, marca_equipos, tipo_equipos, tarifa_dia_equipos, id_equipos];
        }
...
app.put('/updateMaterial/:id_materiales', multer({ storage }).single('foto_material'), (req, res) => {
    ...
    const { id_materiales } = req.params;
    const { descripcion_materiales, tipo_moneda_materiales, unidad_medida_materiales, valor_unitario_materiales, fabricacion_materiales, margen_materiales, costo_unitario_materiales, dimension_materiales, unidad_materiales, precio_producto_materiales, proveedor_materiales } = req.body;
    ...
    db_con.query(getOldImageQuery, [id_materiales], (err, results) => {
        ...
        if (newImagePath) {
            updateQuery = `UPDATE materiales SET
                                ...
                                WHERE
                                    id_materiales = ?`;
            queryParams = [newImagePath, descripcion_materiales, tipo_moneda_materiales, unidad_medida_materiales, valor_unitario_materiales, fabricacion_materiales, margen_materiales, costo_unitario_materiales, dimension_materiales, unidad_materiales, precio_producto_materiales, proveedor_materiales, id_materiales];
        } else {
            updateQuery = `UPDATE materiales SET
                                    descripcion_materiales = ?,
                                    tipo_moneda_materiales = ?,
                                ...
                                WHERE
                                    id_materiales = ?`;
            queryParams = [descripcion_materiales, tipo_moneda_materiales, unidad_medida_materiales, valor_unitario_materiales, fabricacion_materiales, margen_materiales, costo_unitario_materiales, dimension_materiales, unidad_materiales, precio_producto_materiales, proveedor_materiales, id_materiales];
        }
